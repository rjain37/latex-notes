...
\noindent The observation we make is that we win iff edges at the end are min-cut.
...
\begin{itemize}
    ...
    \item Contract $(u, v)$.
    \item Repeat until 2 vertices left.
\end{itemize}
\mlenma{}{For any min-cut C, the probability that Karger's algorithm finds C is $\geq \frac{1}{\poly(n)}$.}
\begin{proof}
...
\noindent \textbf{Idea:} Repeat $t$ times and return best cut we find. (We will determine the value for $t$ below.)
...
\textbf{Exercise:} $O(\log^2 n)$ repetitions suffice.
...
\textbf{Claim:} $P(\text{successful path}) \geq \frac{1}{t}$.
\begin{proof}
...
\thm{Moser's Theorem}{The expected number of fix-its is at most $O\left(\frac nd\right)$. }
\noindent \textbf{Proof Idea:}
Use random bits, for example $0010010100101\ldots$. Construct a ``transcript'' $T$ such that if you tell me $T$, I can recover all the random bits you used. The interesting property of $T$ is that:
\begin{align*}
